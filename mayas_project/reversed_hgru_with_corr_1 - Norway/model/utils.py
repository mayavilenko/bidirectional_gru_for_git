...
def custom_loss(category, category_indent, category_weights, son_parent_dict, y_true, y_pred, last_value, weights_dict, coefficient_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2, loss_coef_3):
    """
    ...
    pred_diff = y_pred-last_value
    true_sign = torch.sign(y_true-last_value)
    desired_shape = torch.mul(pred_diff,true_sign).shape

    sign_term = torch.minimum(torch.zeros(desired_shape), torch.mul(pred_diff,true_sign)) #if the sign of the pred and true val is equal, then add 0, else - add pred_difference (the larger the error, the larger the pennalty to loss)
    ...
    loss = loss*np.exp(-category_indent) #we would like the loss to be larger for upper (smaller) indents so the model will focus more on these categories
...
def training_loop(model, category_indent, train_dataloader, optimizer, category, weights_dict, coefficient_dict, son_parent_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2, loss_coef_3):
    running_loss = 0
    running_mse_loss=0
    ...
    for inputs, labels in train_dataloader:
        ...
        loss = custom_loss(category, category_indent, category_weights, son_parent_dict, labels, pred, last_value, weights_dict, coefficient_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2, loss_coef_3)
...
def evaluation_loop(model, category_indent, test_dataloader, category, weights_dict, coefficient_dict, son_parent_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2, loss_coef_3):
    # Evaluation
    ...
    with torch.no_grad():
        for inputs, labels in test_dataloader:
            ...
            inputs, labels = inputs.to(Device), labels.to(Device)
            out = model(inputs)
            test_batch_loss = custom_loss(category, category_indent, category_weights, son_parent_dict, labels, out, last_value, weights_dict, coefficient_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2, loss_coef_3)
...
def training_and_evaluation(model, category_indent, train_dataloader, test_dataloader, optim, category, weights_dict, coefficient_dict, son_parent_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2, loss_coef_3, path): 
    ...
    for epoch in range(Epochs):
        epoch_train_loss, epoch_train_mse_loss = training_loop(model, category_indent, train_dataloader, optim, category, weights_dict, coefficient_dict, son_parent_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2,loss_coef_3)
        epoch_test_loss, epoch_test_mse_loss= evaluation_loop(model, category_indent, test_dataloader, category, weights_dict, coefficient_dict, son_parent_dict, parent_to_son_list_dict, category_id_to_name_dict, loss_coef_1, loss_coef_2,loss_coef_3)
        ...
        #Display MSE loss on tensorboard
        #writer.add_scalar(tag='mse_loss/train', scalar_value=epoch_train_mse_loss, global_step=epoch)
        #writer.add_scalar(tag='mse_loss/test', scalar_value=epoch_test_mse_loss, global_step=epoch)
