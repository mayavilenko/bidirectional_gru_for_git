...
def training_and_evaluation(model, train_dataloader, test_dataloader, optim, category, parent_weights, loss_coef, path): 
    ...
    for epoch in range(Epochs):
        epoch_train_loss  = training_loop(model, train_dataloader, optim, parent_weights, loss_coef)
        epoch_test_loss = evaluation_loop(model, test_dataloader, parent_weights, loss_coef)
...
def get_results_on_test_set(weights_path, train_dataset_dict, test_dataset_dict, categories = None, seed=42):
    predictions_dict = {}
    ...
    for category in categories:
        ...
        basic_model = GRUModel(input_dim=Features, hidden_dim=HiddenSize, layer_dim=LayersDim, output_dim=OutputDim, dropout_prob=DropoutProb, seed=seed)
        basic_optimizer = Optimizer
        ckp_path = weights_path+'_'+str(seed)+'/'+category+'.pt'
...
def aggregate_predictions(prediction_dicts, categories):
    # Initialize an empty dictionary to store the averaged predictions
    aggregated_predictions = {}

    # Iterate over each category to average predictions
    for category in categories:
        # Stack predictions from each dictionary for the current category
        stacked_predictions = torch.stack([pred_dict[category] for pred_dict in prediction_dicts], dim=0)

        # Compute the mean prediction across the stacked predictions
        mean_prediction = torch.mean(stacked_predictions, dim=0)

        # Store the averaged predictions in the dictionary
        aggregated_predictions[category] = mean_prediction
