...
def predict_multiple_horizons(weights_path,  test_dict, categories, horizons=8):
    prediction_dict = {}
    for category in categories:
        print(category)
        category_test_df = test_dict[category]
        x_test = category_test_df.iloc[:,:-1].to_numpy()
        basic_model = Model
        basic_optimizer = Optimizer
        ckp_path = weights_path+category+'.pt'
        model, _, _, _ = load_checkpoint(ckp_path, basic_model, basic_optimizer)
        ...
        with torch.no_grad():  # No need to track gradients
            for _ in range(horizons):
                x_test = torch.from_numpy(x_test).to(torch.float32)
                x_test = x_test.view(x_test.shape[0], SequenceLength, Features)
                prediction = model(x_test)
                all_predictions.append(prediction)

                # Update current_data for the next prediction
                # Remove the first time step and concatenate the new prediction at the end
                x_test_arr = x_test.squeeze().numpy()
                prediction_arr = prediction.numpy()
                concat_x_test_array = np.concatenate((x_test_arr,prediction_arr), axis=1)
                x_test = concat_x_test_array[:,1:]
    
        prediction_dict[category] = all_predictions
